...
- 错误处理准则：**能恢复就恢复，不能恢复就让它炸**。
  - 可恢复：例如参数验证失败、临时网络错误等，在当前层可以给出合理 fallback 或重试，就地处理并记录上下文。
  - 不可恢复：例如逻辑异常、数据损坏、依赖不可用等，直接抛出让上层处理，禁止“catch-log-继续”把错误藏起来。
  - 捕获异常必须有明确目的（重试、转换异常类型等），否则不要捕获。
...
- **不要在 body 里重复 success 字段或 message 字段**，HTTP 状态码已经表达了成功/失败
- 响应格式：
  ```python
  # ✅ 成功响应 (200/201)
  {
  ...
  ```
- 资源创建场景返回 `201 + {"id": <new_id>}`，仅需提供新资源标识；更新/删除无返回体，使用 `204 No Content`
- WebSocket 用于实时数据推送
- CORS 配置允许前端跨域
